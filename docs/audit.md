...
#### Ошибка сборки AUR пакета
- **Проблема:** PKGBUILD ссылается на `default.conf`, но workflow не включает этот файл в assets
- **Влияние:** Ошибка сборки: `default.conf was not found in the build directory`
- **Статус:** ✅ **ИСПРАВЛЕНО** - добавлен `aur/default.conf` в список assets
- **Приоритет:** ВЫСОКИЙ
